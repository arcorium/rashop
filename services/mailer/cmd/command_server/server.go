...
import (
  ...
  "github.com/arcorium/rashop/services/mailer/internal/infra/client"
  ...
  "github.com/arcorium/rashop/services/mailer/internal/infra/publisher"
...
const serviceName = constant.SERVICE_COMMAND_NAME
...
func (s *Server) setup() error {
  ...
  // Client
  mailClient, err := client.NewSMTP(client.SMTPConfig{
    Host:     s.config.SMTP.Address,
  ...
  kafkaPublisher := publisher.NewKafka(publisher.KafkaTopic{
    DomainEvent:      constant.MailerDomainEventTopic,
    IntegrationEvent: constant.MailerIntegrationEventTopic,
  }, s.producer, serde.JsonSerializer{})

  // DQL
  dlqPublisher := kafka.NewForwarder(constant.MailerDlqTopic, s.producer)

  // Service
  commandConfig := service.DefaultMailCommandConfig(command.CommonHandlerParameter{
    Persistent:  mailRepo,
    MailClient:  mailClient,
    MediaClient: nil, // TODO: Set after implement media service
    Publisher:   kafkaPublisher,
  })
  mailCommandSvc := service.NewMailCommand(commandConfig)

  commandConsumerConfig := service.DefaultMailCommandConsumerConfig(commandConfig.Send)
  commandConsumerSvc := service.NewMailCommandConsumer(commandConsumerConfig)

  // Handler
  // gRPC
  commandHandler := handler.NewMailCommand(mailCommandSvc)
  commandHandler.Register(s.grpcServer)

  // Messaging
  consumerHandler := consumer.NewMailCommandHandler(commandConsumerSvc)
  if err := s.consumerSetup(); err != nil {
    return err
  }

  consumerDispatcher := dispatcher.NewMailCommandConsumerGroup(dlqPublisher, consumerHandler, serde.JsonAnyDeserializer{})
  consumerDispatcher.Run(context.Background(), s.consumer, constant.MailerIntegrationEventTopic)
